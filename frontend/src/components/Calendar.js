...
const Calendar = () => {
  ...
  const resetWeek = async () => {
    if (
      window.confirm(
        'Are you sure you want to reset all assignments for this week? This action cannot be undone.'
      )
    ) {
      try {
        setResetting(true);
        ...
        await axios.delete(
          `/api/admin/reset-assignments?start_date=${startDate}&end_date=${endDate}`
        );
  ...
  return (
    <div>
      ...
      <div className="flex-between mb-3">
        <Button variant="secondary" onClick={goToCurrentWeek} title="Go to current week">
      ...
      <div className="responsive-container">
        <div className="calendar">
          <div className="calendar-header">
            <Button onClick={prevWeek}>{isMobileView ? '← Prev' : '← Previous Week'}</Button>
            <h2>
              {formatDate(daysOfWeek[0])} to {formatDate(daysOfWeek[6])}
            </h2>
            <Button onClick={nextWeek}>{isMobileView ? 'Next →' : 'Next Week →'}</Button>
          </div>
