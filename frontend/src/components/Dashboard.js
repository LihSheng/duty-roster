...
const Dashboard = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const assignmentsRes = await axios.get(
          `/api/assignments?start_date=${today}&end_date=${today}`
        );
  ...
  const markAsCompleted = async (id) => {
    try {
      ...
      setTodayDuties(
        todayDuties.map((duty) => (duty.id === id ? { ...duty, status: 'completed' } : duty))
      );
    ...
    <div>
      ...
      <div className="dashboard-stats mb-3">
        <div className="stat-card">
          ...
          <p>Total Duties</p>
          <Link to="/duties" className="btn btn-primary btn-sm mt-1">
            Manage
          </Link>
        </div>
        ...
        <div className="stat-card">
          ...
          <p>People</p>
          <Link to="/people" className="btn btn-primary btn-sm mt-1">
            Manage
          </Link>
        </div>
        ...
        <div className="stat-card">
          ...
          <Link to="/calendar" className="btn btn-primary btn-sm mt-1">
            View
          </Link>
        </div>
        ...
        <div className="stat-card">
          ...
          <Link to="/admin" className="btn btn-danger btn-sm mt-1">
            Manage
          </Link>
        </div>
      </div>
