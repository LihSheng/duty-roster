...
router.post('/assignment/:id', async (req, res) => {
  ...
  db.get(query, [assignmentId], async (err, assignment) => {
    ...
    try {
      const results = await notificationService.notifyAssignment(person, duty, assignment.assigned_date);
...
router.post('/overdue', async (req, res) => {
  ...
  db.all(query, async (err, assignments) => {
    ...
    for (const assignment of assignments) {
      ...
      try {
        const notificationResults = await notificationService.notifyOverdue(person, duty, assignment.due_date);
        results.push({
